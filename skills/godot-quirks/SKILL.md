...
name: godot-quirks
description: |
  Godot 4 engine-level gotchas and non-obvious behaviors for both C# and GDScript (e.g. nodes silently dropped during scene serialization, collision-layer bitmask vs UI index, GLB material serialization). Some items are C#-only with GDScript callouts. Read before writing Godot 4 C# or GDScript engine-level code.
---

# Known Quirks

Engine-level gotchas discovered through building games. These are Godot bugs and non-obvious behaviors — not language tutorials. They apply to **both C# and GDScript** unless a bullet is tagged `[C# only]` (those carry a short GDScript callout where the analog differs).

- **`SetScript()` disposes the C# wrapper** *(`[C# only]`)* — calling `SetScript()` on a node in a scene builder invalidates the C# managed wrapper. Any subsequent use of that variable throws `ObjectDisposedException`. Build the full node hierarchy first, call `SetScript()` last. For the root node, use a temp parent to re-obtain the reference after `SetScript()`:
  ```csharp
  var temp = new Node();
  var root = new Node2D();
  root.Name = "Main";
  ...
  ```
  *GDScript callout:* `set_script()` does NOT dispose anything (there's no managed wrapper to invalidate), so the temp-parent dance is unnecessary — keep using the same variable.
...
- **`_Ready()` skipped in `_Initialize()`** *(`[C# only]`)* — when running `--script`, `_Ready()` on instantiated scene nodes does NOT fire during `_Initialize()`. Call init methods manually after `Root.AddChild(node)`.
- **`_Process()` signature in SceneTree scripts** *(`[C# only]`)* — must be `public override bool _Process(double delta)` (returns bool), not void.
- **Autoloads in SceneTree scripts** *(`[C# only]`)* — cannot reference autoload singletons by name. Find them via `Root.GetChildren()` and match by `.Name`.
  - *GDScript callout for the three items above:* GDScript SceneTree scripts use `func _initialize()` and `func _process(delta) -> bool`, and autoload singletons ARE reachable directly by their configured global name.
...
- **C# enum names — don't guess, look up** *(`[C# only]`)* — LLM training data is mostly GDScript Godot content, so the model confidently generates plausible but wrong C# enum names (e.g., `BGModeEnum.Sky` instead of `BGMode.Sky`). Always use the `godot-api` skill to verify C# enum names. *GDScript callout:* GDScript enum/constant names also differ from what's guessed (e.g. the `Environment.BG_SKY` constant form) — verify them via the `godot-api` skill too.
...
- **Containers stomp child transforms; 4.7 adds `offset_transform_*`** — a Container overrides its children's position/scale/rotation on every relayout, so animating those (or even editing them in the editor / AnimationPlayer) on container children silently breaks. Pre-4.7 workarounds: dummy wrapper node, or tween anyway and pray no relayout lands mid-tween (also `modulate.a` instead of `visible` — visibility changes trigger relayout). **Godot 4.7+**: use the Control offset transform — `offset_transform_enabled = true`, then `offset_transform_position/_rotation/_scale` (+ `_position_ratio`, `_pivot`/`_pivot_ratio`). It applies on top of the container transform, survives relayout, is editor/AnimationPlayer-editable, and `_pivot_ratio = (0.5, 0.5)` replaces the whole "await size, set pivot_offset, reconnect on resized" dance. Caveat: `offset_transform_visual_only` defaults `true` — input stays at the layout rect; untick when you move something far and still want it clickable at its visual spot. Values set while `enabled = false` have no effect.
...
**Pass-by-value types in functions** *(`[C# only]`)***:**
- `bool`, `int`, `float`, `Vector3`, `Aabb`, `Transform3D` etc. are value types — assigning to a parameter inside a method does NOT update the caller's variable. Use `ref`/`out` or return the modified value:
  ...
  ```
  *GDScript callout:* GDScript passes objects (Nodes, Resources, Arrays, Dictionaries) by reference and built-in math types (`Vector2`, `Transform3D`, etc.) by value too — but reassigning a parameter never affects the caller in either case, so there's no `ref`/`out` struct-copy surprise to work around; just return the modified value.
